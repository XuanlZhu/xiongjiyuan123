...
def f_max_v(sum_money,i,main=[]):
    if sum_money <= 0 or i < 0:
        dic[(sum_money,i,main)] = 0
        return 0
    if sum_money < price(i):
        return f_max_v( sum_money, i - 1, main)
    if is_main(i)==0:
        '''如果是主件，可以自由判断是否购买'''
        max_v = max(f_max_v(sum_money-price(i),i-1,main+[pre_index(i)])+price(i)*value(i),f_max_v(sum_money,i-1,main))
        return max_v
    else:
        '''如果是配置，先判断主件是否购买'''
        if is_main(i)-1 in main:
            '''如果主件已购买，那么配件可以自由判断是否购买'''
            max_v = max(f_max_v(sum_money - price(i), i - 1, main) + price(i) * value(i),f_max_v(sum_money, i - 1, main))
            return max_v
        else:
            '''如果主件没有购买，那么配件不能购买'''
            return f_max_v(sum_money, i - 1, main)
